...
/**
 * 
 * @param {Object} data tf.tensor object
 * @param {Number} batchSize Number of batches
 * @param {Boolean} shuffle shuffle the data or not.. true by default
 */
function tfCreateBatch(data, batchSize, shuffle=true){

  const dataSize  = data.shape[0];
  const batchStep = Math.floor(dataSize/batchSize);
  ...
  // some helpful arrays for slicing the data to create batches..
  const shapeTemplate = new Array(data.shape.length).fill(0);
  const sliceUpTo = shapeTemplate.slice().fill(-1);
  sliceUpTo[0] = batchStep;

  for(let i=0;i<batchSize;i+=batchStep){

    const sliceBegin = shapeTemplate.slice(); // copying the shape template
    sliceBegin[0] = i;

    dataBatches.push( data.slice(sliceBegin, sliceUpTo) );
  }
